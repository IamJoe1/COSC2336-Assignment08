...
/**
 * @brief Insert value on back of list
 *
 *  Insert the indicated on the back of the list.
 * This is a linked list implementation of a list,
 * so we have to create a new mode and insert it correctly
 * onto the end of the linked list.
 *
 * @param value The value of some type T to be added to
 * the back of the linked list of values.
 *
 * @return List<T>& Returns to this LList as the result so
 * that we can again chain insertions to the back of the LList.
 */
template<class T>
List<T>& LList<T>::insertBack(const T& value)
{
...
/**
 * @brief Overload operator>> to insert value on back of list
 *
 *  Insert the indicated on the back of the list.
 * This is a linked list implementation of a list,
 * so we have to create a new mode and insert it correctly
 ...
 */
template<class T>
List<T>& LList<T>::operator>>(const T& value)
{
  return insertBack(value);
}
 ...
 * If it's empyt, an ListEmptyException is thrown.
...
T LList<T>::getFront() const
{
  ...
  {
    ostringstream out;
    out << "Error: <LList::getFront()> Is empty, size:  " << this->size;
 ...
 *
 *  The function first checks if the list is empty.
 * If it's empty, an ListEmptyException is thrown.
...
T LList<T>::getBack() const
{
  ...
  {
    ostringstream out;
    out << "Error: <LList::getBack()> Is empty, size:  " << this->size;
    throw ListEmptyException(out.str());
  }
  return back->value;
}
...
/**
 * @brief Insert value on front of list
 *
 *  Insert the indicated on the front of the list.
 * This is a linked list implementation of a list,
 * so we have to create a new mode and insert it correctly
 * onto the front of the linked list.
 *
 * @param value The value of some type T to be added to
 * the front of the linked list of values.
 *
 * @return List<T>& Returns to this LList as the result so
 * that we can again chain insertions to the front of the LList.
 */
template<class T>
List<T>& LList<T>::insertFront(const T& value)
{
  Node<T>* newNode = new Node<T>;
  newNode->value = value;
  newNode->next = front;
...
/**
 * @brief Overload operator<< to insert value on front of list
 *
 *  Insert the indicated on the front of the list.
 * This is a linked list implementation of a list,
 * so we have to create a new mode and insert it correctly
...
template<class T>
void LList<T>::deleteIndex(int index)
{
  if (index < 0 or index >= this->size)
  {
    ostringstream out;
    out << "Error: <Alist>::deleteIndex() memory bounds exception";
    throw ListMemoryBoundsException(out.str());
    return;
  }
  if (index == 0)
